...
import defaultImage from "../../../assets/images/network-devices.jpg";
import "./IT_Network_Inventory.css";

const IT_Network_Inventory = () => {
  ...
  useEffect(() => {
    const saved = JSON.parse(localStorage.getItem("itInventory")) || [];
    setInventory(saved);
  }, []);
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      case "item_ID":
        if (!/^I[0-9]{3}$/.test(value)) {
          error = "Item ID must start with 'I' followed by 3 digits (e.g., I001)";
        }
        break;
      case "name":
      case "category":
  ...
  const handleSubmit = (e) => {
    ...
    const newItem = { ...formData, image: formData.image || null };
    ...
    if (isEditing) {
      ...
      localStorage.setItem("itInventory", JSON.stringify(updated));
      ...
      localStorage.setItem("itInventory", JSON.stringify(newInventory));
  ...
  const generateReport = () => {
    ...
    const doc = new jsPDF();
    doc.text("IT & Network Inventory Report", 20, 10);
    ...
    doc.save("IT_Network_Inventory_Report.pdf");
  };
  ...
  const exportCSV = () => {
    ...
    link.setAttribute("download", "IT_Network_Inventory.csv");
  ...
  return (
    <div className="emergency-equipment-container">
      ...
      <h2>IT & Network Equipment Inventory</h2>

      {/* Top Default Banner */}
      <div className="topic-image-container">
        <img src={defaultImage} alt="IT Equipment" className="topic-image" />
      </div>
      ...
      {/* Table Section */}
      <h3>IT Equipment Inventory List</h3>
...
export default IT_Network_Inventory;
